...
(function(){
  ...
  var q=queryType();
  var current=byKey(q)?q:(byKey(stored())?stored():(byKey(root.getAttribute('data-type'))?root.getAttribute('data-type'):DEFAULT));
  if(byKey(q)) store(q);
    ...
    return ''+
      '<div class="wrap">'+
        ...
        '<div class="site-footer__fine">'+
          ...
          '<p class="sf-type">Type · <button type="button" data-open-type aria-controls="type-pop">'+esc(byKey(current).name)+'</button></p>'+
        '</div>'+
      '</div>';
  }
